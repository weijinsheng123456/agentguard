            ...
            for pattern in AI_COMMENT_PATTERNS:
                if re.search(pattern, stripped, re.IGNORECASE):
                    issues.append(Issue(
                        ...
                        line=i,
                        code="AI_PLACEHOLDER",
                        message=f"AI placeholder comment: {stripped[:60]}",
                        severity=Severity.INFO,
                        rule_name=self.name,
                        fixable=False,
                    ))
            ...
            for pattern in SUSPICIOUS_MODULES:
                if re.match(pattern, stripped):
                    issues.append(Issue(
                        ...
                        message=f"Suspicious module name: {stripped}",
                        severity=Severity.INFO,
                        rule_name=self.name,
                        fixable=False,
                    ))
                    ...
                    if hasattr(node, 'end_lineno') and hasattr(node, 'lineno'):
                        ...
                        if func_lines > 200:
                            issues.append(Issue(
                                ...
                                message=f"Function {node.name} too long ({func_lines} lines), possibly AI-generated",
                                severity=Severity.INFO,
                                rule_name=self.name,
                                fixable=False,
                            ))
