            ...
            for node in ast.walk(tree):
                    ...
                    if has_interpolation:
                        ...
                        if not has_params:
                            issues.append(Issue(
                                ...
                                line=node.lineno,
                                code="SQLI_FSTRING",
                                message=f"f-string SQL query in {func}() — SQL injection risk",
                                severity=Severity.BLOCKER,
                                rule_name=self.name,
                                fixable=False,
                            ))
                ...
                # % 格式化检测
                    ...
                    issues.append(Issue(
                        ...
                        line=node.lineno,
                        code="SQLI_PERCENT",
                        message=f"SQL with % formatting in {func}() — use parameterized query",
                        severity=Severity.BLOCKER,
                        rule_name=self.name,
                        fixable=False,
                    ))
                ...
                # .format() 调用
                    ...
                    if self._is_format_call(first_arg):
                        issues.append(Issue(
                            ...
                            line=node.lineno,
                            code="SQLI_FORMAT",
                            message=f"SQL with .format() in {func}() — use parameterized query",
                            severity=Severity.BLOCKER,
                            rule_name=self.name,
                            fixable=False,
                        ))
                ...
                # 字符串变量拼接（如 query + condition）
                    ...
                    issues.append(Issue(
                        ...
                        line=node.lineno,
                        code="SQLI_CONCAT",
                        message=f"SQL string concatenation in {func}() — injection risk",
                        severity=Severity.BLOCKER,
                        rule_name=self.name,
                        fixable=False,
                    ))
